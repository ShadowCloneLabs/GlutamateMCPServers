...
import { DEFAULT_PORT, getConfiguredPort } from "./config.js";
...
loadingTimeout = setTimeout(() => {
  Logger.error("Loading timeout reached. Switching to safe mode (lazy loading...)");
...
async function attemptEagerLoading() {
  try {
    Logger.error("Attempting to eagerly load modules...");
    
    // Try to import all modules
    contacts = (await import('./utils/contacts.js')).default;
    Logger.error("- Contacts module loaded successfully");
    
    notes = (await import('./utils/notes.js')).default;
    Logger.error("- Notes module loaded successfully");
    
    message = (await import('./utils/message.js')).default;
   Logger.error("- Message module loaded successfully");
    
    mail = (await import('./utils/mail.js')).default;
    Logger.error("- Mail module loaded successfully");
    
    reminders = (await import('./utils/reminders.js')).default;
    Logger.error("- Reminders module loaded successfully");

    webSearch = (await import('./utils/webSearch.js')).default;
    Logger.error("- WebSearch module loaded successfully");
    
    calendar = (await import('./utils/calendar.js')).default;
   Logger.error("- Calendar module loaded successfully");
    
    maps = (await import('./utils/maps.js')).default;
    Logger.error("- Maps module loaded successfully");
    ...
    Logger.error("All modules loaded successfully, using eager loading mode");
    initServer();
  } catch (error) {
...
const sseServer = new AppleSSEServer();
